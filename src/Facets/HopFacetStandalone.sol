...
contract HopFacetStandalone is ILiFi, ReentrancyGuard, SwapperV2, Validatable {
    /// Storage ///
        ...
        refundExcessNative(payable(msg.sender)) //! I would opt to remove this: saves 135 gas only though
    {
        ...
        // depositAsset2(_bridgeData.sendingAssetId, _bridgeData.minAmount);
    ...
    function _startBridge(ILiFi.BridgeData memory _bridgeData, HopData memory _hopData) private {
        ...
        if (block.chainid == 1) {
            // Ethereum L1
            bridge.sendToL2{ value: sendingAssetId == address(0) ? _bridgeData.minAmount : 0 }(
            ...
            bridge.swapAndSend{ value: sendingAssetId == address(0) ? _bridgeData.minAmount : 0 }(
                _bridgeData.destinationChainId,
                _bridgeData.receiver,
                _bridgeData.minAmount,
                _hopData.bonderFee,
