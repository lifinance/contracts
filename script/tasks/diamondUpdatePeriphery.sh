...
register() {
  ...
  while [ $ATTEMPTS -le "$MAX_ATTEMPTS_PER_SCRIPT_EXECUTION" ]; do
    ...
    if [[ "$DEBUG" == *"true"* ]]; then
      ...
      cast send "$DIAMOND" 'registerPeripheryContract(string,address)' "$CONTRACT_NAME" "$ADDR" --account $(getAccount "$NETWORK" "$ENVIRONMENT") --rpc-url "${!RPC}" --legacy
      ...
      cast send "$DIAMOND" 'registerPeripheryContract(string,address)' "$CONTRACT_NAME" "$ADDR" --account $(getAccount "$NETWORK" "$ENVIRONMENT") --rpc-url "${!RPC}" --legacy >/dev/null 2>&1
    fi
