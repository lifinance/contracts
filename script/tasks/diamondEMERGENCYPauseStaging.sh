...
function diamondEMERGENCYPauseStaging {
  echo ""
  echo "[info] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> running script diamondEMERGENCYPauseStaging now...."
  # load env variables
  source .env
  ...
  local ENVIRONMENT="staging" # this script is only meant to be used on PROD diamond
  ...
  echo ""
  echoDebug "in function diamondEMERGENCYPauseStaging"
  ...
  sendMessageToSlackSmartContractsChannel "TEST TEST TEST ON STAGING :warning: an emergency diamond action was just triggered (action: $ACTION, user info: $USER_INFO). Please immediately investigate if this action was not planned. :warning:"
  ...
  echo "[info] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< script diamondEMERGENCYPauseStaging completed"
}
...
function handleNetwork() {
  ...
  DIAMOND_ADDRESS=$(getContractAddressFromDeploymentLogs "$NETWORK" "staging" "$DIAMOND_CONTRACT_NAME")
