    ...
    if [[ ! ${#NEW_DEXS[@]} -eq 0 ]]; then
      ...
      while [ $ATTEMPTS -le "$MAX_ATTEMPTS_PER_SCRIPT_EXECUTION" ]; do
        ...
        if [[ "$DEBUG" == *"true"* ]]; then
          ...
          cast send "$DIAMOND_ADDRESS" "batchAddDex(address[])" "${PARAMS[@]}" --rpc-url "$RPC_URL" --account $(getAccount "$NETWORK" "$ENVIRONMENT") --legacy
        else
          # do not print output to console
          cast send "$DIAMOND_ADDRESS" "batchAddDex(address[])" "${PARAMS[@]}" --rpc-url "$RPC_URL" --account $(getAccount "$NETWORK" "$ENVIRONMENT") --legacy >/dev/null
        fi
