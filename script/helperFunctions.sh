...
function logContractDeploymentInfo {
  ...
  if [[ "$existing_entry" == "null" ]]; then
    jq --arg CONTRACT "$CONTRACT" \
      ...
      --arg VERIFIED "$VERIFIED" \
      --arg SALT "$SALT" \
      '.[$CONTRACT][$NETWORK][$ENVIRONMENT][$VERSION] += [{ ADDRESS: $ADDRESS, OPTIMIZER_RUNS: $OPTIMIZER_RUNS, TIMESTAMP: $TIMESTAMP, CONSTRUCTOR_ARGS: $CONSTRUCTOR_ARGS, DEPLOYSALT: $SALT, VERIFIED: $VERIFIED }]' \
      "$LOG_FILE_PATH" >tmpfile && mv tmpfile "$LOG_FILE_PATH"
  else
    jq --arg CONTRACT "$CONTRACT" \
      --arg NETWORK "$NETWORK" \
      ...
      --arg VERIFIED "$VERIFIED" \
      --arg SALT "$SALT" \
      '.[$CONTRACT][$NETWORK][$ENVIRONMENT][$VERSION][-1] |= { ADDRESS: $ADDRESS, OPTIMIZER_RUNS: $OPTIMIZER_RUNS, TIMESTAMP: $TIMESTAMP, CONSTRUCTOR_ARGS: $CONSTRUCTOR_ARGS, DEPLOYSALT: $SALT, VERIFIED: $VERIFIED }' \
      "$LOG_FILE_PATH" >tmpfile && mv tmpfile "$LOG_FILE_PATH"
  fi
