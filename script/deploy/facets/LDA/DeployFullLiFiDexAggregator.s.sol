...
contract DeployScript is ScriptBase {
    ...
    function run()
        public
        returns (LiFiDEXAggregatorDiamond deployed, bytes memory constructorArgs)
    {
        ...
        address[] memory facetAddresses = _deployAndAddLDAFacets(address(deployed));
        ...
        emit log_named_address("LiFiDEXAggregatorDiamond deployed at:", address(deployed));
    }
    ...
    function _verifyCoreFacetsExist() internal {
        ...
        for (uint256 i = 0; i < requiredFacets.length; i++) {
            address facetAddress = json.readAddress(string.concat(".", requiredFacets[i]));
            require(facetAddress != address(0), string.concat("Required core facet not found: ", requiredFacets[i]));
    ...
    function _deployLDADiamond() internal returns (LiFiDEXAggregatorDiamond) {
        ...
        emit log_named_address("LiFiDEXAggregatorDiamond deployed successfully at:", address(diamond));
        return diamond;
    }
    ...
    function _addCoreFacets(address ldaDiamond) internal {
        ...
        emit log_named_string("Reading regular deployment file from path:", path);
        ...
        // Build cuts only for facets that don't exist
        ...
        if (!loupeExists) {
            ...
            bytes4[] memory diamondLoupeSelectors = _getCoreFacetSelectors("DiamondLoupeFacet");
            
            cuts = _pushFacetCut(cuts, LibDiamond.FacetCut({
        ...
        if (!ownershipExists) {
            ...
            bytes4[] memory ownershipSelectors = _getCoreFacetSelectors("OwnershipFacet");
    ...
    function _checkIfLoupeExists(address ldaDiamond) internal view returns (bool) {
        // Try to call facetAddresses() - if it works, DiamondLoupeFacet exists
        try DiamondLoupeFacet(ldaDiamond).facetAddresses() returns (address[] memory) {
            return true;
        } catch {
            return false;
        }
    ...
    function _checkIfOwnershipExists(address ldaDiamond) internal view returns (bool) {
        // Try to call owner() - if it works, OwnershipFacet exists
        // We need to use low-level call since we don't have OwnershipFacet interface imported
        (bool success,) = ldaDiamond.staticcall(abi.encodeWithSignature("owner()"));
        return success;
    }
    ...
    function _pushFacetCut(
        ...
        LibDiamond.FacetCut[] memory newCuts = new LibDiamond.FacetCut[](existingCuts.length + 1);
    ...
    function _deployAndAddLDAFacets(address ldaDiamond) internal returns (address[] memory) {
        DiamondCutFacet cutter = DiamondCutFacet(ldaDiamond);
        address[] memory facetAddresses = new address[](0);
        
        // Deploy and add AlgebraFacet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "AlgebraFacet"));
        
        // Deploy and add CoreRouteFacet (special case - returns tuple)
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddCoreRouteFacet(cutter));
        
        // Deploy and add CurveFacet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "CurveFacet"));
        
        // Deploy and add IzumiV3Facet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "IzumiV3Facet"));
        
        // Deploy and add NativeWrapperFacet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "NativeWrapperFacet"));
        
        // Deploy and add SyncSwapV2Facet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "SyncSwapV2Facet"));
        
        // Deploy and add UniV2StyleFacet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "UniV2StyleFacet"));
        
        // Deploy and add UniV3StyleFacet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "UniV3StyleFacet"));
        
        // Deploy and add VelodromeV2Facet
        facetAddresses = _appendFacetAddress(facetAddresses, _deployAndAddSingleFacet(cutter, "VelodromeV2Facet"));
    ...
    function _deployAndAddSingleFacet(string memory facetName) internal returns (address) {
        ...
        if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("AlgebraFacet"))) {
            DeployAlgebraFacetScript deployScript = new DeployAlgebraFacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("CurveFacet"))) {
            DeployCurveFacetScript deployScript = new DeployCurveFacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("IzumiV3Facet"))) {
            DeployIzumiV3FacetScript deployScript = new DeployIzumiV3FacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("NativeWrapperFacet"))) {
            DeployNativeWrapperFacetScript deployScript = new DeployNativeWrapperFacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("SyncSwapV2Facet"))) {
            DeploySyncSwapV2FacetScript deployScript = new DeploySyncSwapV2FacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("UniV2StyleFacet"))) {
            DeployUniV2StyleFacetScript deployScript = new DeployUniV2StyleFacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("UniV3StyleFacet"))) {
            DeployUniV3StyleFacetScript deployScript = new DeployUniV3StyleFacetScript();
            facetAddress = address(deployScript.run());
        } else if (keccak256(abi.encodePacked(facetName)) == keccak256(abi.encodePacked("VelodromeV2Facet"))) {
    ...
    function _getCoreFacetSelectors(string memory facetName) internal returns (bytes4[] memory) {
    ...
    function _saveFacetAddresses(address ldaDiamond, address[] memory facetAddresses) internal {
        emit log("=== UPDATING LDA DEPLOYMENT FILE ===");
        
        // Fix the path construction - the fileSuffix already contains "lda.staging."
        // so we don't need to add another "lda." prefix
        string memory path = string.concat(
            root,
            "/deployments/",
            network,
            ".",
            fileSuffix,  // This already contains "lda.staging."
            "json"
        );
        
        emit log_named_string("Saving to correct path:", path);
        
        // Facet names in the same order as the addresses array
        string[9] memory facetNames = [
            "AlgebraFacet",
            "CoreRouteFacet", 
            "CurveFacet",
            "IzumiV3Facet",
        ...
        // Build the complete JSON content
        string memory jsonContent = "{\n";
        jsonContent = string.concat(jsonContent, '  "LiFiDEXAggregatorDiamond": "', vm.toString(ldaDiamond), '",\n');
        
        for (uint256 i = 0; i < facetAddresses.length; i++) {
            emit log(string.concat("Adding ", facetNames[i], " to JSON at:"));
            emit log_named_address("Address:", facetAddresses[i]);
            
            string memory comma = i < facetAddresses.length - 1 ? "," : "";
            jsonContent = string.concat(
                jsonContent, 
                '  "', facetNames[i], '": "', vm.toString(facetAddresses[i]), '"', comma, '\n'
            );
        }
        
        jsonContent = string.concat(jsonContent, "}");
        
        // Write the complete JSON file at once
        emit log("Writing complete JSON file...");
