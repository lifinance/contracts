...
contract DeployScript is DeployScriptBase {
    ...
    constructor() DeployScriptBase("AoriV2Facet") {}

    function run()
        public
        returns (AoriV2Facet deployed, bytes memory constructorArgs)
    {
        constructorArgs = getConstructorArgs();

        deployed = AoriV2Facet(deploy(type(AoriV2Facet).creationCode));
    }

    function getConstructorArgs() internal override returns (bytes memory) {
        // If you don't have a constructor or it doesn't take any arguments, you can remove this function
        string memory path = string.concat(root, "/config/aoriV2.json");
