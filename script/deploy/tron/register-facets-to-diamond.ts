...
import { getPrivateKey, ENERGY_PRICE, DEFAULT_SAFETY_MARGIN } from './utils'
...
async function estimateDiamondCutEnergy(
  ...
  try {
    ...
    if (result.energy_used) {
      ...
      // Add safety margin (50% buffer)
      const estimatedEnergy = Math.ceil(
        result.energy_used * DEFAULT_SAFETY_MARGIN
      )
      consola.info(
        `📊 Energy with ${DEFAULT_SAFETY_MARGIN}x safety margin: ${estimatedEnergy}`
      )
 ...
 */
async function isFacetRegistered(
  diamond: any,
  facetAddress: string,
  tronWeb: any
): Promise<boolean> {
  try {
    const registeredFacets = await diamond.facetAddresses().call()
    const facetHex = tronWeb.address.toHex(facetAddress)

    return registeredFacets.some((addr: string) => {
      const addrHex = addr.startsWith('0x') ? addr : '0x' + addr
      const facetHexNormalized = facetHex.replace(/^41/, '0x')
      return addrHex.toLowerCase() === facetHexNormalized.toLowerCase()
    })
  } catch {
    return false
  }
...
async function registerFacetsBatch(
  ...
  for (const facetName of facetNames) {
    ...
    const isRegistered = await isFacetRegistered(diamond, facetAddress, tronWeb)
    if (isRegistered) {
  ...
  if (balanceTRX < estimatedCost * 2) 
    throw new Error(
      `Insufficient balance. Have: ${balanceTRX} TRX, Need: ~${(
        estimatedCost * 2
      ).toFixed(4)} TRX`
    )
  ...
  try {
    ...
    const tx = await diamond
      ...
      .send({
        feeLimit: Math.ceil(estimatedEnergy * ENERGY_PRICE),
    ...
    if (error.error) 
      consola.error('Error details:', error.error)
...
async function registerFacetsToDiamond(
  ...
  try {
    ...
    if (!fullHost) 
      throw new Error('Tron RPC URL not found in networks.json')
    ...
    try {
      const facets = await diamond.facetAddresses().call()
      loupeExists = true
      consola.success(
      ...
      for (const facetHex of facets) {
        const facetBase58 = tronWeb.address.fromHex(facetHex)
        const facetName = Object.entries(deployments).find(
          ([_, addr]) => addr === facetBase58
        )?.[0]
        if (facetName) 
          consola.info(`  - ${facetName}: ${facetBase58}`)
    ...
    if (!options.dryRun) {
      ...
      try {
        const registeredFacets = await diamond.facetAddresses().call()
        consola.success(
          `📋 Total registered facets: ${registeredFacets.length}`
        )

        // Map addresses to names and display
        const facetList: string[] = []
        for (const facetHex of registeredFacets) {
          const facetBase58 = tronWeb.address.fromHex(facetHex)
          const facetName = Object.entries(deployments).find(
            ([_, addr]) => addr === facetBase58
          )?.[0]
          ...
          if (facetName) {
            ...
            // Get selector count for this facet
            const selectors = await diamond
              .facetFunctionSelectors(facetBase58)
              .call()
            consola.info(
        ...
        const expectedFacets = Object.keys(FACET_SELECTORS)
        ...
        if (missingFacets.length > 0) 
          ...
          consola.success('✨ All expected facets are registered!')
