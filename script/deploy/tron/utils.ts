 ...
 *
 * This function checks multiple environment variable names for backward compatibility:
 * 1. TRON-PRO-API-KEY (preferred, matches TronGrid header name)
 * 2. TRON_PRO_API_KEY (alternative format)
 * 3. TRON_API_KEY (alternative)
 * 4. TRONGRID_API_KEY (backward compatibility)
 *
...
export function getTronGridAPIKey(verbose = false): string | undefined {
  ...
  for (const envVarName of envVarNames) {
    try {
      ...
      // Try next environment variable name
      continue
    }
  }

  // Also check process.env directly for alternative formats
  for (const envVarName of envVarNames) {
    ...
    if (apiKey) {
      ...
      consola.warn(
        '⚠️  Using TronGrid RPC but no API key found. ' +
          'Set TRON-PRO-API-KEY, TRON_API_KEY, or TRONGRID_API_KEY in .env to avoid rate limiting.'
      )
    }
  }
