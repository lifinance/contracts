...
export async function deletePendingProposals(
  ...
  for (const h of hashes) {
    const doc = await pendingTransactions.findOne({ network: { $eq: network }, safeTxHash: { $eq: h } })
    if (!doc) {
    ...
    const res = await pendingTransactions.deleteOne({ network: { $eq: network }, safeTxHash: { $eq: h } })
    consola.success(
