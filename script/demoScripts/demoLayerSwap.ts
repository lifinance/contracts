...
import { LayerSwapFacet__factory as LayerSwapFacet } from '../../typechain/factories/LayerSwapFacet.sol/LayerSwapFacet__factory'
...
async function main() {
  ...
  const { client, publicClient, walletAccount, lifiDiamondAddress, lifiDiamondContract } = await setupEnvironment(srcChain, LAYER_SWAP_FACET_ABI)
  ...
  // import config from '../../config/layerSwap.json'
  //
  ...
  const layerSwapData: LayerSwapFacet.LayerSwapDataStruct = {
    // Add your specific fields for LayerSwap here.
  }

  // === Start bridging ===
  await executeTransaction(
    () =>
      lifiDiamondContract.write.startBridgeTokensViaLayerSwap(
        [bridgeData, layerSwapData],
        // { value: fee } optional value
      ),
    'Starting bridge tokens via LayerSwap',
    publicClient,
    true
  )
}
