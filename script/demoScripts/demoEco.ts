...
import { EcoFacet__factory as EcoFacet } from '../../typechain/factories/EcoFacet.sol/EcoFacet__factory'
...
async function main() {
  ...
  const { client, publicClient, walletAccount, lifiDiamondAddress, lifiDiamondContract } = await setupEnvironment(srcChain, ECO_FACET_ABI)
  ...
  // import config from '../../config/eco.json'
  //
  ...
  const ecoData: EcoFacet.EcoDataStruct = {
    // Add your specific fields for Eco here.
  }

  // === Start bridging ===
  await executeTransaction(
    () =>
      lifiDiamondContract.write.startBridgeTokensViaEco(
        [bridgeData, ecoData],
