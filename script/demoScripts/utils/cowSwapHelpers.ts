...
import { findHexValueOccurrences } from './patcher'
...
export async function setupCowShedPostHooks(config: CowShedPostHooksConfig) {
  ...
  const minAmountNeedle = `0x${randomBytes(32).toString('hex')}`
  ...
  const needlePositions = findHexValueOccurrences(
    relayCalldata,
    minAmountNeedle
  )

  if (needlePositions.length === 0) {
    throw new Error('Could not find minAmount position in calldata')
  }

  const minAmountOffset = BigInt(needlePositions[0])
  consola.info(
  ...
  const patcherCalldata = encodeFunctionData({
    ...
    args: [
      usdcAddress as `0x${string}`, // valueSource - USDC contract (single address)
      valueGetter, // valueGetter - balanceOf call (single bytes)
