...
contract ArbitrumBridgeFacetTest is TestBaseFacet {
    ...
    function initiateBridgeTxWithFacet(bool isNative) internal override {
        if (isNative) {
            arbitrumBridgeFacet.startBridgeTokensViaArbitrumBridge{ value: bridgeData.minAmount + addToMessageValue }(
                bridgeData,
                arbitrumData
            );
        } else {
            arbitrumBridgeFacet.startBridgeTokensViaArbitrumBridge{ value: addToMessageValue }(
                bridgeData,
                arbitrumData
            );
        }
    ...
    function initiateSwapAndBridgeTxWithFacet(bool isNative) internal override {
        if (isNative) {
            ...
            arbitrumBridgeFacet.swapAndStartBridgeTokensViaArbitrumBridge{ value: addToMessageValue }(
                bridgeData,
                swapData,
                arbitrumData
            );
