...
contract ReceiverCelerIMTest is DSTest, DiamondTest {
    ...
    ReceiverCelerIM receiver = new ReceiverCelerIM(address(USDC_WHALE), CBRIDGE_MESSAGE_BUS_ETH, executorAddress);
    ...
    function setUp() public {
        ...
        executorAddress = address(new Executor(address(USDC_WHALE), address(0)));
        receiver = new ReceiverCelerIM(address(USDC_WHALE), CBRIDGE_MESSAGE_BUS_ETH, executorAddress);
    }

    function testCanSetNewMessageBusAddressAsOwner() public {
        vm.expectRevert(UnAuthorized.selector);
        vm.startPrank(DAI_WHALE);
        receiver.setCBridgeMessageBus(CBRIDGE_MESSAGE_BUS_POLY);
        assertEq(receiver.cBridgeMessageBusAddress(), CBRIDGE_MESSAGE_BUS_ETH);
        vm.stopPrank();
        // vm.startPrank(0xb4c79daB8f259C7Aee6E5b2Aa729821864227e84); // deployer
        vm.startPrank(USDC_WHALE); //TODO QUESTION: Why is this account the owner? See https://book.getfoundry.sh/forge/writing-tests
        receiver.setCBridgeMessageBus(CBRIDGE_MESSAGE_BUS_POLY);
        assertEq(receiver.cBridgeMessageBusAddress(), CBRIDGE_MESSAGE_BUS_POLY);
    }
    ...
    //TODO still failing - needs fixing
    ...
    function testCanExecuteArbitraryMessage() internal {
        ...
        swapDataDest[0] = LibSwap.SwapData({
            ...
            fromAmount: amountOut,
            callData: abi.encodeWithSelector(usdc.transferFrom.selector, USDC_WHALE, finalReceiver, amountOut),
            requiresDeposit: false
        });
