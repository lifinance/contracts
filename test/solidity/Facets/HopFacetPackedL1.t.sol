...
contract HopFacetPackedL1Test is TestBase {
    ...
    function testBase_StoreConstructorParametersCorrectly() public override {
        HopFacetPacked standaloneHopFacetPacked = new HopFacetPacked(
            address(this),
            address(0)
        );

        assertEq(
            standaloneHopFacetPacked.nativeBridge(),
            address(0),
            "nativeBridge address doesn't match"
        );
        assertEq(
            standaloneHopFacetPacked.nativeL2CanonicalToken(),
            address(0),
            "nativeL2CanonicalToken address doesn't match"
        );
        assertEq(
            standaloneHopFacetPacked.nativeHToken(),
            address(0),
            "nativeHToken address doesn't match"
        );
        assertEq(
            standaloneHopFacetPacked.nativeExchangeAddress(),
            address(0),
            "nativeExchangeAddress address doesn't match"
        );
        assertEq(
            standaloneHopFacetPacked.owner(),
            address(this),
            "wrong owner"
        );
    }
