...
contract StargateFacetTest is TestBase {
    ...
    function setUp() public {
        ...
        functionSelectors[1] = stargateFacet.startBridgeTokensViaStargate.selector;
        functionSelectors[2] = stargateFacet.swapAndStartBridgeTokensViaStargate.selector;
        ...
        functionSelectors[7] = stargateFacet.setFunctionApprovalBySignature.selector;
    ...
    function initiateBridgeTxWithFacet(bool isNative) internal override {
        if (isNative) {
            stargateFacet.startBridgeTokensViaStargate{ value: bridgeData.minAmount + addToMessageValue }(
                bridgeData,
                stargateData
            );
        } else {
            stargateFacet.startBridgeTokensViaStargate{ value: addToMessageValue }(bridgeData, stargateData);
        }
    }

    function initiateSwapAndBridgeTxWithFacet(bool isNative) internal override {
        if (isNative) {
            stargateFacet.swapAndStartBridgeTokensViaStargate{ value: swapData[0].fromAmount + addToMessageValue }(
                bridgeData,
                swapData,
                stargateData
            );
        } else {
            stargateFacet.swapAndStartBridgeTokensViaStargate{ value: addToMessageValue }(
                bridgeData,
                swapData,
                stargateData
            );
