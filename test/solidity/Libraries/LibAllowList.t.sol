...
contract LibAllowListTest is Test {
    ...
    function test_ContractReferenceCountingWithMultipleSelectors() public {
        ...
        assertTrue(
            ...
            "New: Selector A should be allowed"
        );
        assertTrue(
            LibAllowList.contractSelectorIsAllowed(
                address(testContract),
            ...
            "New: Selector B should be allowed"
        );
        assertEq(
            LibAllowList
        ...
        assertFalse(
            ...
            "New: Selector A should be removed"
        );
        assertTrue(
            LibAllowList.contractSelectorIsAllowed(
                address(testContract),
        ...
        assertEq(
            LibAllowList
                ...
                .length,
            1,
            "New: Contract should have 1 selector left"
        );
        // Backward Compatibility State
        assertTrue(
            LibAllowList.contractIsAllowed(address(testContract)),
            "BC: Contract should STILL exist due to reference count"
        );
        assertEq(
            LibAllowList.getAllowedContracts().length,
            1,
            "BC: Still 1 contract in global list"
        );
        assertFalse(
            LibAllowList.selectorIsAllowed(SELECTOR_A),
            "BC: Selector A should exist"
        ...
        assertEq(
            LibAllowList.getAllowedSelectors().length,
            1,
            "BC: Should only be 1 selector in global list"
        );
        ...
        assertFalse(
            ...
            "New: Selector B should be removed"
        );
        assertEq(
            LibAllowList
                .getWhitelistedSelectorsForContract(address(testContract))
                .length,
            0,
            "New: Contract should have 0 selectors left"
        );
        // Backward Compatibility State
        assertFalse(
            LibAllowList.contractIsAllowed(address(testContract)),
            "BC: Contract should NOT be allowed"
        );
        assertEq(
            LibAllowList.getAllowedContracts().length,
            0,
        ...
        assertFalse(
            LibAllowList.selectorIsAllowed(SELECTOR_A),
            "BC: Selector A should NOT exist"
        );
        assertFalse(
            LibAllowList.selectorIsAllowed(SELECTOR_B),
            "BC: Selector B should NOT exist"
        );
        assertEq(
            LibAllowList.getAllowedSelectors().length,
            0,
            "BC: Should only be 0 selectors in global list"
        );
    }
    ...
    function test_SelectorReferenceCountingAcrossMultipleContracts() public {
        ...
        assertEq(
            LibAllowList.getAllowedContracts().length,
            2,
            "BC: There should be 2 contracts in global list"
        );
        assertTrue(
            LibAllowList.selectorIsAllowed(SELECTOR_A),
        ...
        assertEq(
            LibAllowList.getAllowedSelectors().length,
            0,
            "BC: Only 0 selectors in global list"
        );
    }
    ...
    function test_MultipleContractsWithOverlappingSelectors() public {
        ...
        assertEq(
            LibAllowList
                ...
                .length,
            2,
            "New: Contract should have 2 selectors left"
        );
        assertEq(
            LibAllowList
                .getWhitelistedSelectorsForContract(address(contract2))
        ...
        assertEq(
            LibAllowList.getAllowedSelectors().length,
            3,
            "BC: Only 3 selectors in global list"
        );
        ...
        assertTrue(
            LibAllowList.selectorIsAllowed(SELECTOR_B),
            "BC: Selector should exist from contract2"
        );
        assertTrue(
            LibAllowList.selectorIsAllowed(SELECTOR_C),
            "BC: Selector should exist"
    ...
    function test_IterableListMaintainsCorrectOrderAfterRemoval() public {
        ...
        bytes4[] memory selectors = LibAllowList
            .getWhitelistedSelectorsForContract(address(testContract));
        ...
        selectors = LibAllowList.getWhitelistedSelectorsForContract(
            address(testContract)
        );
        ...
        assertEq(
            selectors[1],
            SELECTOR_C,
            "Last element should have replaced the middle one"
        );
    }
    ...
    function _assertStateSyncedAndCorrect(
        ...
        assertEq(
            LibAllowList.contractSelectorIsAllowed(
                _params.contractAddr,
                _params.selector
            ),
            _params.pairShouldExist,
            "New: Granular pair check failed"
        );
        assertEq(
            LibAllowList
                .getWhitelistedSelectorsForContract(_params.contractAddr)
                .length,
            _params.localSelectorCount,
            "New: Local selector count mismatch"
        );

        // Check Backward Compatibility State
        assertEq(
            LibAllowList.contractIsAllowed(_params.contractAddr),
            _params.contractShouldExistInBC,
            "BC: Global contract check failed"
        );
        assertEq(
            LibAllowList.selectorIsAllowed(_params.selector),
            _params.selectorShouldExistInBC,
            "BC: Global selector check failed"
        );
        assertEq(
            LibAllowList.getAllowedContracts().length,
            _params.globalContractCount,
            "BC: Global contract count mismatch"
        );
        assertEq(
            LibAllowList.getAllowedSelectors().length,
            _params.globalSelectorCount,
            "BC: Global selector count mismatch"
        );
    }
}
