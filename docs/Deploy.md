...
A production deploy is refused unless the working tree matches the commit it will be recorded at and that commit was shown present in a repository a verifier could fetch it from, because a deployment record only claims anything if someone can rebuild at its commit. Presence is read from a remote-tracking ref when this clone has one — which shows the remote this clone fetches from holds the commit, without establishing that the record names that remote — and asked of the declared repository otherwise, so a commit whose branch was since deleted, and a shallow clone, no longer refuse an honest deploy - while a commit no repository can be shown to hold still does. Asking the declared repository needs `gh` installed and authenticated and only covers `github.com` remotes; without either, presence is UNKNOWN and the deploy is refused as before. Only build-affecting paths count - `src/`, `lib/`, `foundry.toml`, `remappings.txt` and `foundry.lock`; changes under `deployments/`, `broadcast/` and `script/` are ignored. Staging deploys warn instead of refusing. To see the verdict without deploying:
