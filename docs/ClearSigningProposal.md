...
3. **`destinationChainId` as `chainId`, except where a LI.FI id collides** — `chainId` is the ERC-7730 v2 integer format that renders an EIP-155 chain id as the network's name, so the wallet shows "Polygon" instead of `137`. Most LI.FI non-EVM ids (Solana, BTC, Sui, …) are far outside the allocated range and simply degrade to digits. `LIFI_CHAIN_ID_HYPERCORE` is the exception: it is `1337`, which chain registries resolve to **"Geth Testnet"** — so a Hypercore bridge would name a wrong chain on the signing screen. Bridges that can reach it (Mayan, Unit, PolymerCCTP, AcrossV4Swap) therefore keep `raw`. The generator derives that set from the facet sources and refuses to run if a new sub-`1e12` id appears in `LiFiData.sol` unacknowledged.
