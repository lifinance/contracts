...
4. **The sign-time codehash gate** (`codehash-sign-gate.ts`, deciding through
   ...
   instead of reporting a pass. And calldata that decodes to nothing installing
   code is outside its scope and says so; a proposal with empty calldata prints
   no gate line, because there is nothing to judge. `diamondUpdatePeriphery` is
   in neither set: it is not among the selectors this decoder opens, so a
   proposal carrying it reaches the gate as an unopened frame and the gate makes
   no claim about it.
...
- **Bytecode of anything a proposal does not install.** The sign-time codehash
  ...
  to infer it. `diamondUpdatePeriphery` is not among the selectors it decodes,
