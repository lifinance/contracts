...
| Propose | Timelock-wrapped proposals dedup too: the `scheduleBatch` salt is derived from the action (chain, timelock, targets, payloads), not the clock, so re-proposing the same wrapped work yields the same `intentHash`. The timelock is asked whether that operation id already exists — a **pending** hit warns that the proposal duplicates live work, an **executed** hit moves to the next deterministic salt so a legitimate repeat does not revert after signing | Warn (pending) / auto-advance (executed) / refuse after 16 attempts | `deriveTimelockSalt` + `pickTimelockSalt` in `safe-utils.ts` |
