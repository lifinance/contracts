...
| Propose | In-flight nonce uniqueness per Safe: two concurrent proposers cannot mint the same nonce (partial unique index over `pending` + `submitted`) | Block insert, re-run required | `unique_inflight_safe_nonce` index in `safe-utils.ts`; diagnose with `report-nonce-collisions.ts` |
