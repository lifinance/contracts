...
(`checkBlindSigningEnabled` in `ledger.ts`). Before any of that, a network
preflight (`signer-preflight.ts`) probes each candidate network's RPC endpoints
and refuses the ones that are unset, unreachable, or answering for the wrong
chain: a refused network is printed once under `CANNOT START` and is never
graded, and a run where no network can start ends at exit `78` without ever
reaching the review below (§5). Per pending transaction the signer sees:
...
| Confirm | Network preflight, ahead of every other Confirm check: before the ownership reads and before a check ledger exists, each candidate network is probed once — `ETH_NODE_URI_<NET>` must be set, the endpoints it names must answer `eth_chainId`, and the answer must equal the chain id `config/networks.json` declares for that network. The third is the dangerous case: a wrong-chain endpoint answers every later read truthfully, about a different chain. The probe reads through the whole endpoint set (`ETH_NODE_URI_<NET>` plus its `_FALLBACKS`, via `buildFallbackReadClient`), the same list the executability gate reads, so a dead primary with healthy fallbacks is not refused; the all-networks path's ownership filter (`getNetworksWithActionableTransactions`) still reads the primary alone through `buildReadOnlyClient`, so that agreement stops at the preflight. An explicit `--rpcUrl` replaces the configured list but does **not** excuse an unset variable — the chain object is resolved from the env var before the override is read. Probes run concurrently, each bounded by `PROBE_TIMEOUT_MS` (5s) rather than viem's own timeout, so several unreachable networks refuse in seconds instead of reading as a hang; findings are reported in the order the networks were given, and a node's error text is sanitised and URL-redacted before it is printed. A refused network is named once with its cause and its remedy under a red `CANNOT START` heading, is never graded, and stays out of the check ledger's denominator — its checks are never enumerated as unverified rows. **Not a gate:** the gate letters describe the proposal and red there means do not sign, which an unset environment variable is not, so this carries no gate letter and no ledger row. **It closes one cause of a screen full of unverified rows, not the class** — `eth_chainId` is the most permissive method a node serves, so an endpoint that answers it while rate-limited or method-restricted on `eth_call` passes here and fails the checks behind it anyway | Block (per network, ungraded). No network can start → exit `78` with no ledger built at all; a partial run grades the networks that can start, then prints a line after the ledger naming what the verdict does not cover and exits `78` as well. `78` is `EX_CONFIG` from `sysexits`, deliberately distinct from a refusal to sign, so a wrapper script or runbook step can tell "fix your environment" from "this proposal was rejected" without parsing output | `signer-preflight.ts` (`networkPreflight` / `renderNetworkPreflight`, `PREFLIGHT_EXIT_CODE`), wired in `confirm-safe-tx.ts` ahead of the ownership filter; probes through `buildFallbackReadClient` in `read-only-safe-client.ts` |
...
| Confirm | Calldata address check: every address a proposal's cuts reference — `Add`/`Replace` targets, removal targets, and the `_init` delegatecall target — resolved against the deployment record, with the role deciding whether failing to account for it is a refusal or a warning. A mistake-catcher, not a lie-catcher: the record is written by the deploying machine, so this catches the typo, the address copied from another chain, and the contract nobody deployed. Carries **no ledger row** — its only decidable source is the deploy log, which the ledger treats as reporting-only, and no committed file yet supplies the address→identity expectations an anchor would need. | Display / warn only | `calldata-address-check.ts` + `calldata-address-collector.ts` (EXSC-699, wired EXSC-994) |
