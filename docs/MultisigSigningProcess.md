...
| Confirm | Sign-time codehash gate: every address a decoded `diamondCut` installs — `Add`/`Replace` targets plus a non-zero `_init` — must match a local rebuild at the commit its production deployment record names, under the `foundry.toml` profile that network's config selects — the profile *name* is what is pinned here, and the compiler pair it resolves to is whatever that profile pinned at the commit being rebuilt. **Not** an ancestry check against `main` — per D3 the referenced commit need only be present and fetchable, so this row is anchored differently from the Propose row above it. MATCH passes; MISMATCH and UNVERIFIABLE both block and stay distinct. A removal-only cut carrying `_init` is refused rather than gated. A MATCH with bytes excluded as immutables is downgraded to UNVERIFIABLE until WP-2.3 checks their values, and calldata the decoder cannot open makes **no claim** rather than reporting a pass. On zkEVM networks the values are read from `ImmutableSimulator` instead (gate L), and which slots exist and what they are called comes from a vanilla-solc `--ast` build of the same commit, in the same detached checkout layer 1 matched — never the signer's working tree, where dropping an `immutable` keyword would grade as nothing to check. A network resolving to more than one build lineage, a contract that build does not define, defines in more than one source file or only outside `src/`, or a build that fails (including for want of that commit's solc, since it runs `--offline`), refuses rather than grading as having no immutables, and a slot with no registry entry blocks as it does on an inlining chain. Those values are graded against `immutableRegistry.json`, `deployRequirements.json` and `config/` read at the same pinned `origin/main` commit as the target state, never the signer's checkout: an immutable `main` does not declare stays UNVERIFIABLE until its registry entry merges. The gate's own code still runs from the checkout, so a deploy PR's branch is kept from supplying the expectation its own deployment is graded against only when the signing tool is `main`'s — sign from `main`, never from the deploy PR's branch. Asserted on both the sign and the execute route, not only at signature time — a proposal already at threshold is broadcast through a different funnel. Infrastructure failures block too, in two places: one that stops any verdict being reached (config unreadable) is a refusal, while one that stops a single address being judged (RPC or attestation store unreachable) is that address's `UNVERIFIABLE` verdict. **Tron** is graded like every other chain, with three differences it is worth knowing about: its source is fetched from the `contracts-tron` remote rather than `origin` ([TronFork.md](./TronFork.md)) and a clone that has no such remote, or one pointing anywhere else, is refused with the `git remote add` to run; its records key addresses in base58 while the cut carries hex, so both spellings are looked up; and most of its production records carry no commit at all, so a cut installing one of those is `UNVERIFIABLE` and stays that way until the record is fixed (count them with `db.production.countDocuments({network:'tron', gitCommitHash:{$not:/^[0-9a-f]{40}$/}})`) | Block | `codehash-sign-gate.ts` + `codehash-sign-gate-deps.ts` in `confirm-safe-tx.ts`, deciding through `script/deploy/codehash/` (EXSC-906) |
...
| Confirm | Target state graded against `origin/main`, never the reviewer's checkout: the anchor is `git show origin/main:script/deploy/_targetState.json` after a fresh fetch of an explicit `+refs/heads/main:refs/remotes/origin/main` (git updates that ref only opportunistically, so a clone without a covering refspec would otherwise read a stale anchor with no error), read through `refs/remotes/origin/main` in full rather than the short name, which git resolves through tags and heads first, so a tag a proposer's clone carries cannot shadow the fetched ref. `origin` must be `github.com/lifinance/contracts` over https or SSH or the read refuses — a fork remote would let a proposer author the expected state, and over cleartext `http://` an on-path attacker could serve both the state and the SHA anchoring it; the URL is taken from `git remote get-url`, which applies any `insteadOf` rewrite and so reports where a fetch would really go. Which branch the reviewer happens to be on cannot change the expected state it grades against — though the check itself runs from the checkout, so that holds only with `main`'s signing tool. Graded per case, not as one equality. The expected version depends on what the target state declares: a network reading `latest` (the normal case) is graded against the contract's `@custom:version` **at the same pinned ref**, so a cut installing an older version refuses as a downgrade and one installing the repo's version passes; a network carrying a **version pin** is graded as equality against that pin, so anything else — newer included — refuses as `pinned-mismatch`, because a pin states which version must be there rather than a floor. A version pair that cannot be ordered, a proposed version no deployment record resolves, and a `latest` entry whose source cannot be read at the pinned ref (a deleted or untagged contract) all refuse. A **first-time add** has no entry on `main` by construction (the target-state PR merges only after execution, so gating on it would deadlock) and is labeled "not previously targeted" with the count of networks declaring that contract, without blocking — intent there rests on the linked ticket and PR; a **removal** is reported only. See [docs/TargetState.md](TargetState.md). The statuses that may proceed are named, so an unrecognised cut action, a cut whose calldata cannot be read, a facet address no deployment record names on that network (the same address on another chain is deliberately not consulted — the mirror carries one address as two different contracts across networks), a deployment record that contradicts itself about which contract or version an address is (the `(network, address)` pair is not unique in that cache — six such pairs today, two at genuinely different versions), an anchor read through the wrong remote, and an anchor that could not be refreshed all refuse. | Block (downgrade / pinned mismatch / unorderable / expected version unresolved / proposed version unresolved / unidentified / ambiguous record / unreadable / anchor unavailable) / label (first-time add) / report (removal) | `pinned-target-state.ts` + `diamond-cut-calls.ts`, gated in `confirm-safe-tx.ts` after the nonce gates and before the acknowledgement is recorded (EXSC-704). Each verdict also lands on a run-level `check-ledger.ts` ledger via `confirm-check-registry.ts` (`checkId: target-state`, section `Intent`, class `semantic`), reduced worst-first to one row per network. The run renders that ledger's verdict last, after the signing decisions. Only a `pass` counts toward the verified coverage, and a correct `Add`/`Replace` cut grades `needs-ack` on the target-state row, so a clean rollout closes as an acknowledgement rather than fully green (EXSC-993, EXSC-994) |
