...
| Propose / Confirm | Ledger account selection. A non-integer `--accountIndex` is refused everywhere the flag is offered — `confirm-safe-tx.ts`, `propose-to-safe.ts`, `add-safe-owners-and-threshold.ts`, `ledger-flex-calibrate.ts`, `cleanUpProdDiamond.ts`. The stricter pairing rules — `--accountIndex` refused without `--ledgerLive` (`getLedgerAccount` reads it only on that path), `--derivationPath` refused empty — hold on `confirm-safe-tx.ts`, `add-safe-owners-and-threshold.ts` and `cleanUpProdDiamond.ts` only; `propose-to-safe.ts` and `ledger-flex-calibrate.ts` still accept an `--accountIndex` their derivation ignores, and read `--ledgerLive` from citty's parsed args, where the kebab spelling of `false` arrives truthy | Block | `parseAccountIndex` in `safe-utils.ts`, read from argv via `cli-flags.ts` |
