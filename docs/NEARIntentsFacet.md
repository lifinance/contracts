...
The backend signature is an EIP-712 signature over the quote parameters (transactionId, minAmount, receiver, depositAddress, destinationChainId, sendingAssetId, deadline, quoteId, minAmountOut, destinationAsset) to ensure the deposit address was genuinely generated by the NEAR Intents Bridge API and that the quote settles in the asset the user committed to. The signed type string is:
