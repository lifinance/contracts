...
struct NEARIntentsData {
  ...
  address depositAddress;   // EVM address to send tokens (generated by Bridge API)
  uint256 deadline;         // Unix timestamp when quote expires
  uint256 minAmountOut;     // Minimum output amount on destination
  bytes32 nonEVMReceiver;   // Set only for non-EVM chains (e.g., Solana address as bytes32)
