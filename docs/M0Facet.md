...
| `destinationChainId != 1399811149` → `InvalidCallData`. That is M0's own id for Solana; LI.FI's `LIFI_CHAIN_ID_SOLANA` is the one to send. Passed raw it survives the `uint32` cast untouched and takes the EVM branch above, opening a Solana-bound order with a left-padded EVM recipient and no `BridgeToNonEVMChainBytes32`                          | —                                     |
