...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Facet validation (`InvalidCallData`, `InformationMismatch`, `InvalidNonEVMReceiver`, `InvalidReceiver`, `InvalidAmount`, `NotAnAddress`)                                                                                                                                                                                               | Revert before any transfer — caller keeps everything                                                                                                                                |
...
| EVM: `tokenOut` and `solver` are left-padded addresses → `NotAnAddress`. The OrderBook narrows both with `TypeConverter.toAddress` only when a solver fills — `solver` unconditionally, ahead of the open-to-all `bytes32(0)` case — so a value with non-zero high bytes opens and escrows here and then reverts every fill, stranding the deposit until `fillDeadline`. `bytes32(0)` passes, so an open-to-all order still works. Non-EVM destinations are exempt — an SPL mint and a Solana solver pubkey both use all 32 bytes | —                                     |
| `destinationChainId != 1399811149` → `InvalidCallData`. That is M0's own id for Solana; LI.FI's `LIFI_CHAIN_ID_SOLANA` is the one to send. Passed raw it survives the `uint32` cast untouched and takes the EVM branch above, opening a Solana-bound order with a left-padded EVM recipient and no `BridgeToNonEVMChainBytes32`. `_resolveDestination` owns both the translation and this rejection, so a non-EVM chain M0 enables later cannot gain one without the other                                                        | —                                     |
...
- **For Solana**:
  ...
  - Set `bridgeData.destinationChainId` to `LIFI_CHAIN_ID_SOLANA`
    ...
    Sending `1399811149` directly reverts `InvalidCallData` — it would survive the `uint32`
    cast untouched and take the EVM branch of the receiver binding
