...
other. `FOUNDRY_PROFILE=solc_floor forge build -o /path/to/main-london/out` produces the london
artifacts; that profile carries no `skip`, so it also compiles `script/**` for a `forge script` run.
