...
5. Note any contract-specific deployment requirements and add them to `script/deploy/resources/deployRequirements.json`. Hold each external protocol address in a `public immutable` and annotate its arg with `getter`, so the health check verifies the bound address against config on every chain. Declare every immutable the contract introduces in `script/deploy/resources/immutableRegistry.json` too: CI fails on one without an entry, and the signing gate cannot verify it
