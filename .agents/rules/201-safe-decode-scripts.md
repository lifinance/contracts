...
- **Adding a new chain**: Implement both `IChainExecutor` and `IChainCaller`, add to the factory, and add a network key guard alongside `isTronNetworkKey()` in `@lifi/tron-devkit` (or create a new key guard) — that guard lives in the package, so it is a publish-and-bump there, not a local edit. Consumer scripts should require zero changes.
