...
- **Inside the signer view** (`confirm-safe-tx.ts`): `buildCalldataEffectLines(data, context)` from [script/deploy/safe/calldata-effect-lines.ts](../../script/deploy/safe/calldata-effect-lines.ts). It **returns lines** and writes nothing, so the caller places the block at its own column inside THE CALLDATA DOES. Context is `{ network, indent, target }`. It renders under zone 1's rule — it states, it never grades — and it collapses the fields §2 of the zone-1 design found undecidable, naming every one of them in a `not shown:` roster rather than dropping them.
