...
- **PR-ready (mandatory final step before opening or updating a PR)**: Run `/pr-ready` to execute a local CodeRabbit review against the current branch and resolve all actionable findings before pushing. This applies to humans and agents alike. Do **not** run `gh pr create`, mark a draft as Ready for Review, or push new commits to an open Ready-for-Review PR until the local review is clean (or remaining items are explicitly documented in the PR body). The pre-PR gate (`.claude/scripts/pr-ready-gate.py`, registered as a `PreToolUse` hook in `.claude/settings.json`) enforces this mechanically for agents on `gh pr create`, `gh pr ready`, and `git push` when the current branch has an open non-draft PR. Pushes on draft / WIP branches stay frictionless. See `.agents/commands/pr-ready.md` for the full workflow. Do not bypass with `--no-verify` or similar escape hatches.
