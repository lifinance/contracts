...
- **No Bun-only APIs in shipped modules** ([CONV:NODE-RUNTIME-APIS]). `tsx` runs on **Node**, so anything under `script/**` or `tasks/**` that is not a `*.test.ts` must avoid `import.meta.dir`, `import.meta.main` and `Bun.*`. For the module's own directory use `dirname(fileURLToPath(import.meta.url))` (see `script/tasks/checkDeploymentAddressConsistency.ts`); for a CLI entry guard, call `runMain` unconditionally rather than gating on `import.meta.main`. Test files are exempt — `bun test` provides these APIs. **Neither the type checker nor the test suite will catch a violation**: `tsconfig.json` declares `"types": ["node", "bun"]`, so `tsc` believes the Bun globals exist everywhere, and tests run under Bun where they really do. `import.meta.dir` fails loudly (`join()` throws `ERR_INVALID_ARG_TYPE` at import time, taking down every CLI that transitively imports the module — EXSC-964); `import.meta.main` fails silently (the guard is falsy, so the CLI exits 0 doing nothing). `script/bun-only-api-placement.test.ts` is the backstop for the `import.meta.dir` case.
