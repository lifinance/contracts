...
- **No Bun-only runtime APIs in shipped modules** ([CONV:NODE-RUNTIME-APIS]). A module under `script/**` or `tasks/**` that is not a `*.test.ts` runs on **Node** via `tsx`; only tests run under `bun test`, which is why a test passing proves nothing here. Use the Node equivalent:
  ...
  - `import.meta.main` → `isEntrypoint(import.meta.url)` from `script/utils/is-entrypoint.ts`. Node implements it, but `tsx` leaves it `undefined` for a `.ts` entry on every Node version, so a CLI guarded by it exits 0 having done nothing. Do not hand-roll `process.argv[1] === fileURLToPath(import.meta.url)` either: `import.meta.url` is already symlink-resolved while argv[1] is not, so the compare is `false` whenever a path component is a link; the helper realpaths both sides. Do not call `runMain` unconditionally in a module a test imports — it would run the CLI during `bun test`.
  - The type check (`script/utils/typecheck-files.sh`) checks shipped modules against Node-only types, and `bun lint:node-runtime` refuses all of the above across the tree — including `import.meta.main`, which `@types/node` declares and the type check therefore cannot see.
