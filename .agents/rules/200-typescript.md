...
- **No Bun-only runtime APIs in shipped modules** ([CONV:NODE-RUNTIME-APIS]). `tsx` runs on **Node**, so a module under `script/**` or `tasks/**` that is not a `*.test.ts` cannot rely on Bun's runtime globals. Test files are exempt — `bun test` provides them. **Neither the type checker nor the test suite catches a violation**: `tsconfig.json` declares `"types": ["node", "bun"]`, so `tsc` believes the Bun globals exist everywhere, and tests run under Bun where they really do. The three cases fail differently and have different fixes:
  - `import.meta.dir` — Bun-only, never implemented in Node, so it is `undefined` under `tsx` on every Node version. A module-scope `join(import.meta.dir, …)` throws `ERR_INVALID_ARG_TYPE` **at import time**, taking down every CLI that transitively imports it (that is how EXSC-964 broke `confirm-safe-tx.ts` for the whole team). Use `dirname(fileURLToPath(import.meta.url))` — see `script/tasks/checkDeploymentAddressConsistency.ts`. `script/bun-only-api-placement.test.ts` holds this at zero.
  - `import.meta.main` — broken on our invocation path on **every** Node version, not version-dependent. Node does implement it (22.18+), but `tsx` drops it for `.ts` entry modules: measured `undefined` under the pinned tsx 4.23.13 on Node 18.20.8, 22.23.2 and 26.8.1 alike. The loader decides this, not the Node version — the same tsx reports `true` for a `.mjs` entry, as does a bare `node file.ts` on Node 24+. Since every CLI here is a `.ts` file run via `bunx tsx`, CI does not cover this: a guard spelled `import.meta.main` is a **silent** `exit 0` that does no work, for everyone — that is EXSC-1039, where `bun healthcheck` and the daily `healthCheckAllNetworks` sweep both ran zero invariants and reported success. For a CLI entry guard call `isEntrypoint(import.meta.url)` from `script/utils/is-entrypoint.ts`; it is the only spelling in the tree and `script/entrypoint-guard-placement.test.ts` holds both `import.meta.main` and hand-rolled `process.argv[1]` reads at zero. Do **not** hand-roll the compare. `process.argv[1] === fileURLToPath(import.meta.url)` answers `false` for a direct run whenever a path component is a link, because Node resolves symlinks as it loads while argv[1] stays as typed — the same silent `exit 0`, just harder to reproduce; a `process.argv[1].endsWith('cli.ts')` variant fails the other way and fires for any entry sharing that basename. The helper realpaths both sides, and `is-entrypoint.test.ts` pins the symlinked-file and symlinked-directory cases. Do **not** call `runMain` unconditionally: most guarded modules are imported by their own `*.test.ts` for their pure helpers, and an unconditional call would execute the CLI during `bun test`. `safe/execute-pending-timelock-tx.ts` is the one module left deliberately unguarded, so that no guard can silence the scheduled executor; logic there that needs coverage moves to `safe/timelock-prefetch.ts`.
  - `Bun.*` (`Bun.file`, `Bun.write`, `Bun.TOML`) — throws `ReferenceError: Bun is not defined` at the **call site** under `tsx`. Nine shipped modules use it, `script/utils/utils.ts` among them; every call sits inside a function body, so importing those modules is safe and only invoking the affected helper fails. Don't add new `Bun.*` to anything reachable from a `bunx tsx` entry point — use `node:fs/promises` (`readFile` / `writeFile`) instead. Porting the existing callers is tracked in EXSC-971.
...
- **No re-export barrels**: Import values and types from the module where they are defined.
  - Do not use `export { ... } from '...'`, `export * from '...'`, or `export type { ... } from '...'` solely to re-export another module.
  - Do not add `export { importedSymbol }` at the end of a file when that symbol is only imported from elsewhere; consumers should import from the defining module instead.
  - `export { name }` / `export type { Name }` that reference symbols **declared in the same file** (grouping exports) are fine.
...
## External API fetch and timeout ([CONV:FETCH-TIMEOUT])

- **All outbound HTTP requests to external APIs** (e.g. 4byte/Sourcify, relay APIs, explorer APIs, webhooks) **MUST** use the shared `fetchWithTimeout()` helper from `script/utils/fetchWithTimeout.ts`.
- **Do not use unbounded `fetch(url)`** for external API calls; slow or hung requests can block scripts indefinitely.
- **Standard pattern**: `fetchWithTimeout(url, init?, timeoutMs?)` — uses an AbortController and clears the timeout in a `finally` block. Default timeout is 10 seconds (`DEFAULT_FETCH_TIMEOUT_MS`). Pass a custom `timeoutMs` only when the use case requires it (e.g. large payloads).
- When adding or modifying code that calls external HTTP APIs, use `fetchWithTimeout` and handle `AbortError` or timeout in catch blocks as appropriate (e.g. return null, warn, or retry).
...
- **Type placement**:
  - **Shared or could be shared** (cross-script, multi-chain, or likely to be imported outside one narrow area): `script/common/types.ts`.
  - **Tron-only** (types/interfaces used exclusively for Tron deploy/helpers, not as general script protocol types): `script/deploy/tron/types.ts`.
  - **Script-specific**: inline in the script file if only used locally.
...
### Address normalization ([CONV:ADDR-NORMALIZE])

- When converting raw addresses (base58 `T…`, hex `41…`, or `0x…`) to viem `Address`, use `normalizeAddressForNetwork()` from `script/utils/normalizeAddressStringForViem.ts`. It handles Tron base58 detection automatically and always returns a checksummed viem `Address`.
...
Add comments only where the code doesn't speak for itself. Avoid restating what the code does — explain *why* or *when*.

- **Module header**: Every new TS file MUST open with a `/** … */` block that states: (1) what the module does, (2) who should use it / when to import it. One to three sentences max.
- **Exported functions/classes**: Use JSDoc with `@param`, `@returns`, and `@throws` for every exported function. Include a `@remarks` or leading sentence only when the behaviour is non-obvious (e.g. side effects, retry semantics, chain-specific restrictions).
...
Editor diagnostics (and Cursor “lints”) are **not sufficient** to claim a TypeScript script is clean.
...
If changes touch shared helpers or wide surfaces (or you can’t easily isolate changed files), prefer running the broader checks:
