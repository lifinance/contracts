...
| `manage-wallet-funds.md` | `/manage-wallet-funds <bridge\|swap\|send> --wallet <role\|0x> ...` | Bridge / swap / send funds from any `.env` wallet via the LI.FI API — same-wallet bridge & swap run autonomously, sending to a different recipient needs an explicit human `--confirm`; wallet resolved by role or address (EVM) |
