...
| `create-merged-typings.md` | `/create-merged-typings [PR numbers \| topic keywords]` | Combine several in-flight PRs into one beta typings package for backend/QA — throwaway "DO NOT MERGE" branch merges the PRs on top of main, labeled PR fires Types Bindings, captures the published beta tag, returns the backend pin, then auto-deletes the branch |
...
| `multisig-rollout.md` | `/multisig-rollout <Contract> \| --propose-only <Contract> \| --whitelist-pr <N>` | Orchestrate a production rollout: deploy (`deploy-contract`), propose-only (`proposeContractToNetworks.sh` for already-deployed bytecode), or whitelist-sync; Safe proposals, draft PR when needed, signing hand-off, signature verification, `#dev-sc-multisig-proposals` thread; PolymerCCTP chainId→domain propagation when adding a chain/corridor |
...
| `108-vault-wrapper.md`        | Vault Wrapper subsystem constraints (standalone, not Diamond)    | ❌ On match | `src/VaultWrapper/**`, `test/solidity/VaultWrapper/**`, `script/deploy/vaultWrapper/**`                       |
...
| `203-signing-stack-doc-currency.md` | Gate changes update `docs/MultisigSigningProcess.md` in the same PR | ❌ On match | `script/deploy/safe/**/*.ts`, `script/deploy/codehash/**/*.ts`                                            |
