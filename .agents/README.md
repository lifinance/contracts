...
| `multisig-rollout.md` | `/multisig-rollout <Contract> \| --whitelist-pr <N>` | Orchestrate a production rollout: deploy (delegated to `deploy-contract`) or whitelist-sync across chains, Safe proposals, opt-in stale-facet removal, draft PR, signing hand-off, signature verification, `#dev-sc-multisig-proposals` thread; propagates PolymerCCTP chainId→domain mappings to all live chains when a chain/corridor is added |
