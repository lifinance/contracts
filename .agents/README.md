...
| `finish-rollout.md` | `/finish-rollout <slack thread link>` | Finish a production rollout after timelock execution — verify (timelock queue + on-chain), close the `#dev-sc-multisig-proposals` thread, sync diamond logs onto the rollout PR, `/pr-ready` → ready → `/post-pr-for-review`. The tail of `multisig-rollout` |
