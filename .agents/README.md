...
| `create-merged-typings.md` | `/create-merged-typings [PR numbers \| topic keywords]` | Combine several in-flight PRs into one beta typings package for backend/QA — throwaway "DO NOT MERGE" branch merges the PRs on top of main, labelled PR fires Types Bindings, captures the published beta tag, returns the backend pin, then auto-deletes the branch |
...
| `multisig-rollout.md` | `/multisig-rollout <Contract> \| --whitelist-pr <N>` | Orchestrate a production rollout: deploy (delegated to `deploy-contract`) or whitelist-sync across chains, Safe proposals, opt-in stale-facet removal, draft PR, signing hand-off, signature verification, `#dev-sc-multisig-proposals` thread; propagates PolymerCCTP chainId→domain mappings to all live chains running PolymerCCTPFacet when a chain/corridor is added |
