...
description: Deploys a facet or periphery contract (the version currently in the repo) to one or more networks and registers it in each network's LiFiDiamond — CREATE3 deploy, explorer verification, diamondCut (facets) or diamondUpdatePeriphery (periphery), plus the diamond allowlist sync for diamond-called periphery. This is the staging/testnet deploy path, and the deploy primitive `multisig-rollout` calls. Scope — staging and testnet targets only, or delegation from `multisig-rollout`; a deploy landing in a production (mainnet) diamond goes through `multisig-rollout` instead, which owns the Safe-proposal lifecycle that registration there requires. Tron (`tron`/`tronshasta`) goes to `deploy-contract-tron` — Foundry cannot deploy there. Requires Foundry, gh, and (production only) lifi-connect for MongoDB.
