...
- **Force a fresh build before deploying**: run `forge clean && forge build` (not a bare `forge build`, which can skip files it thinks are unchanged). The deploy scripts derive the CREATE3 salt from `out/<Contract>.sol/<Contract>.json`'s `bytecode.object` via bash — with no rebuild step of their own — before invoking `forge script`. Stale `out/` artifacts (e.g. a worktree fast-forwarded past commits that changed the contract source without an intervening build) silently re-derive the salt from the *old* bytecode. CREATE3 then reports "already deployed" at a pre-existing — possibly superseded — address, and the diamond gets cut to point at outdated bytecode, with **no error surfaced anywhere in the run**. Caught 2026-07-03 on `PaxosTransitFacet`: a staging redeploy meant to ship a refund-routing fix instead silently re-registered the pre-fix facet. Verify by decoding the deploy log's `Predicted Address` / `Contract is already deployed` line against `git log -1 -- src/Facets/<Contract>.sol` — if the predicted address matches an address from an *earlier* commit's deploy log entry, the build was stale.
