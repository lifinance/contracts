...
description: Sweeps all native gas from a rotated-OUT SC wallet to its replacement across every active EVM chain, via `script/tasks/moveNativeFundsToNewWallet.ts` — this is how every wallet rotation funds the new wallet, and the `rotate-*` skills call it for that. Previews per-network balances first (read-only via `cast balance`; the script itself has no dry-run flag), derives the sender from the private key rather than `config/global.json`, requires a human confirmation on the pre-sweep report, and reports per-network moved/skipped/failed. Scope — SC-owned wallets only (deployer / dev / pauser), never refund / feeCollector / withdraw; not for funding a brand-new wallet from scratch, and not for sending gas to an arbitrary recipient, which is `manage-wallet-funds` in `send` mode (single-chain). EVM sweep only — Tron native is a documented manual step. Requires Foundry (`cast`), `bun`, and `jq`.
