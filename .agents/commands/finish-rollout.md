...
- A correlated row is `status: "blocked"` → the pre-execute guard refused the batch and the
  timelock op is still un-executed. The gate fails; do not treat it as done. Re-run the
  lister with `--attention` to confirm it is still executable on-chain, read `statusReason`,
  and follow the remediation it names. `requeue-timelock-op.ts` re-drives the row once the
  cause is cleared; a fully-obsolete folded removal instead needs the op cancelled and the
  primary cut re-proposed alone (see `docs/DeferredDiamondCleanupQueue.md`).
