...
3. **Test Coverage Preservation** (for bridge facets only)
   ...
   - **Verify new tests**: Run `forge test` to ensure newly added adapted tests pass (required per `.agents/rules/099-finish.md`)
...
7. **Remaining Occurrences Review**
   ...
   - **⚠️ Deploy-log entries**: What decides is whether the facet is **still registered on-chain**, per network — not whether its source was deleted, and not whether the parked task (step 6) has retired. So: while the facet is still routed by the loupe, the `deployments/<network>.json` facet→address entry and the `deployments/<network>.diamond.json` entry both **stay** — including when the parked task retired as `cancelled`: cancellation is an operator decision that proves nothing about routing, so the entries keep following the loupe (`superseded` is different: the loupe already confirmed the facet gone via another route, so its entries go per this same routing rule). Once that network's removal has **executed**, delete both entries in the PR that records the execution — the logs mirror current on-chain registration, not deployment history ([docs/DeploymentLogs.md](../../docs/DeploymentLogs.md)). The drain itself is address-keyed and no longer needs the log entry, but the health check's stale-facet invariant maps on-chain addresses to names through it, so pruning a still-registered facet silently degrades that coverage. The weekly reconcile job reports which entries are safe to prune; remove those in a follow-up PR.
...
- Or say "deployments only" to remove only from deployment log files — but **never** the `deployments/*.json` facet→address entry of any facet still registered on the diamond; those go once that network's removal has executed on-chain
...
3. ⚠️  On-chain removal (step 6): parked into the deferred diamond-cleanup queue
   ...
   entries until those parked tasks retire (executed/cancelled/superseded) — then
   clean both deployments/*.json and deployments/*.diamond.json per network whose
   removal EXECUTED, not before and not never.
...
- **Test verification**: Run `forge test` after all changes to ensure ALL tests pass (required per `.agents/rules/099-finish.md` - tests must pass after any Solidity changes, including test file modifications)
