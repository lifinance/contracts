...
.cursor/commands/*.md        ← SOURCE OF TRUTH for commands (edit here)
  ↑ symlinked from:
...
- Uniqueness checks before committing:
  ...
  - `name:` fields are unique within `.agents/rules/` and within `.cursor/commands/`.
...
1. Create `.cursor/commands/<name>.md` with the command content.
2. Create the Claude Code skill bridge:
   ```bash
   mkdir -p .claude/skills/<name>
   ln -sf "../../../.cursor/commands/<name>.md" ".claude/skills/<name>/SKILL.md"
   ```
3. Verify: `ls -l .claude/skills/<name>/SKILL.md` should show the symlink arrow.
...
- **Skill symlinks**: `ls -l .claude/skills/*/SKILL.md` — all should point into `.cursor/commands/`.
- **README accuracy**: `.cursor/rules/README.md` table reflects all files in `.agents/rules/`.
- **Stale references**: grep `.agents/rules/` and `.cursor/commands/` for references to removed files.
